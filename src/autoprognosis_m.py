...
from src.tabular_gated_fusion.tabular_gated_fusion_predict_prob import tabular_gated_fusion_predict_prob
...
from src.tabular_gated_fusion.tabular_gated_fusion_training import tabular_gated_fusion_training
...
class AutoprognosisM:
    ...
    def run(
        ...
        for i, _model_config in tqdm(
            ...
            elif model_config.type == "tabular_gated_fusion":
                tabular_gated_fusion_training(model_config, train_df, val_df, force=force)
    ...
    def fit(
        ...
        for model_config in tqdm(
            ...
            if model_config.type == "imaging":
                ...
                # Run tabular-gated fusion prediction
                prop_val_dfs.append(tabular_gated_fusion_predict_prob(model_config, df=df))
            elif model_config.type == "early_fusion":
    ...
    def predict(self, df: pd.DataFrame, weights: np.ndarray, return_probs=False) -> pd.DataFrame:
        ...
        for config in tqdm(configurations, desc="Predicting"):
            ...
            elif config.type == "tabular_gated_fusion":
                # Run tabular-gated fusion prediction
                prob_values_df = tabular_gated_fusion_predict_prob(config, df=df)
...
# Changes:
# - Add Tabular-Gated-Fusion as another fusion choice to this pipeline
