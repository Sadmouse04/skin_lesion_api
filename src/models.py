...
# NEW: Tabular-Gated Attention Module
class TabularGatedAttention(torch.nn.Module):
    def __init__(self, image_dim, tabular_dim, hidden_dim=256):
        super().__init__()
        
        # Project image features down to the shared latent space
        ...
        # Project tabular features to the shared latent space
        self.tabular_proj = torch.nn.Sequential(
            torch.nn.Linear(tabular_dim, hidden_dim),
            torch.nn.BatchNorm1d(hidden_dim),
            torch.nn.ReLU()
        )
        
        # Gating Network (Generates mask from tabular data)
        self.gating_network = torch.nn.Sequential(
            torch.nn.Linear(hidden_dim, hidden_dim),
            torch.nn.Sigmoid()
        )

        # Force the gate to start "open" (close to 1.0) so the features flow perfectly on Epoch 1. (hardcode the bias as 1.5)
        torch.nn.init.constant_(self.gating_network[0].bias, 1.5)
        
    def forward(self, f_img, f_tab):
        # Project both to hidden dim
        v_proj = self.image_proj(f_img)
        t_proj = self.tabular_proj(f_tab)
        
        # Generate mask
        attention_mask = self.gating_network(t_proj)
        
        # Apply mask to projected image features
        v_gated = v_proj * attention_mask
        
        # Concatenate gated image features with original tabular features
        fused_features = torch.cat([v_gated, f_tab], dim=1)
        
        return fused_features, attention_mask


# NEW: TGFClassifier (Tabular-Gated Fusion)
class TGFClassifier(LightningModule):
    def __init__(
        self,
        imaging_model,
        tabular_input_size,
        ...
        activation_fn=torch.nn.ReLU,
        dropout_prob=0.5,
        hidden_gate_dim=256, # New parameter for the gating dimension
        **kwargs,
    ):
        super().__init__()
        self.imaging_model = imaging_model
        ...
        # New: Initialize the Tabular-Gated Attention Module
        self.gated_attention = TabularGatedAttention(
            image_dim=imaging_model_hidden_size, 
            tabular_dim=tabular_input_size, 
            hidden_dim=hidden_gate_dim
        )

        # The new combined size is the hidden_gate_dim + original tabular size
        combined_input_size = hidden_gate_dim + tabular_input_size
        ...
        # Update the sequential model tracker to include the new attention module
        self.model = torch.nn.Sequential(
            OrderedDict(
                [
                    ("imaging_model", self.imaging_model),
    ...
    def forward(self, image, tabular_data):
        ...
        # New: Dynamic Tabular-Gated Fusion instead of static concat
        fused_features, attention_mask = self.gated_attention(hidden_state, tabular_data)
        
        # Pass the fused features to the final classifier
        logits = self.classifier(fused_features)

        # We return the attention mask in the namespace so you can use it for Grad-CAM later!
        return SimpleNamespace(logits=logits, hidden_states=hidden_state, attention_mask=attention_mask)
...
# Changes:
# - Add Tabular-Gated Fusion mechanisms into the model's choice
# - Add TGFClassifier and TabularGatedAttention classes
