...
from src.tabular_gated_fusion.tabular_gated_fusion_dataset import TGFDataset
from src.models import ImageTabularClassifier
from src.models import TGFClassifier
...
def load_TGF_model(config, df: pd.DataFrame):
    experiment_dir, checkpoint_dir = assemble_experiment_path(config)
    ...
    model = TGFClassifier.load_from_checkpoint(
        best_checkpoint,
        imaging_model=imaging_model,
        tabular_input_size=len(config.feature_columns),
        num_labels=dataset.num_classes(),
...
def interpretability_vision(config: SimpleNamespace, df: pd.DataFrame) -> go.Figure:
    ...
    if config.type == "imaging":
        ...
        model, dataset = load_TGF_model(config, df)
    ...
    for index, series in tqdm(
        ...
        elif config.type in ["tabular_gated_fusion","joint_fusion", "early_fusion"]:
...
# Changes:
# - Add the Intepretability of TGF inside this file
